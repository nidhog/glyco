...
def read_from_csv(file_path,
                  ...
                  interpolate=True, interp_met='polynomial', interp_ord=2):
...
def plot_percentiles(df, lbl, percentiles, group_by=HLBL, color = 'green'):
    ...
    begin_plot()
    mean, dev, med, perc_l, perc_h = get_percentile_dist(df, percentiles, lbl, group_by)
    plt.plot(med)
    for i in range(len(percentiles)):
        plt.fill_between(med.index, perc_l[i], perc_h[i], color=color, alpha=0.2, label='Ho')
    plt.title('Trend of {} for the percentiles: {} as well as {}'.format(lbl,
                                                           ', '.join([str(int(i*100)) for i in percentiles]),
